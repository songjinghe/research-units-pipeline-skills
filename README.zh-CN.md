...
把研究工作流组织成可复用的 pipeline 和语义化 skills。

这个仓库面向这样一类问题：普通 prompt 不够稳，而一堆脚本又太僵硬。这里把研究任务写成分阶段的 pipeline，并为每个阶段定义明确的工件、检查点和 guardrails，这样流程可以中断后恢复、可以审计、也可以持续迭代改进。
