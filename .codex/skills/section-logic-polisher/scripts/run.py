...
def _has_thesis(paragraph: str) -> bool:
    ...
    thesis_patterns = [
        ...
        r"(?i)\bwhat\s+most\s+separates\b.+?\bis\b",
        r"(?i)\b.+?\bstarts?\s+to\s+cohere\s+once\b",
        r"(?i)\bthe\s+main\s+issue\s+is\s+whether\b",
        r"(?i)\bthe\s+important\s+question\s+is\s+which\b",
